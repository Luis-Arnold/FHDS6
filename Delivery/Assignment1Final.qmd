...
df_3 <- cbind(df_2[, !factor_cols], dummy_vars)  # Keep non-factor columns and add dummy variables
...
correlation_matrix <- cor(df_3, use = "pairwise.complete.obs")  # Use pairwise complete observations
```
...
# Based on the picture, it looks like only "loan_amnt" does not have outliers
